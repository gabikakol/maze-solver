...
5. **visualize** <br/>
Class ```Visualize()``` is called by either ```Tremaux()``` or ```WallFollower()``` when exit of the maze is found. It uses PyGame library to visualize the maze's grid and solution path in the PyGame window. 
