...
## Performance
The graphs below show relation between maze size and time taken by each of the algorithm. It is measured assuming the maze is square, ie width == height. Grid size is the number of cells the maze consists of (both walkable paths and walls). It is calculated by: ```width*height```. Hence, the dimensions can be calculated by taking the root of the grid size from the graph analysis. For example, if grid size equals 25: root of 25 is 5, hence width = 25 and height = 25. Minimum grid size is 9 (calculated by 3 * 3) and maximum grid size is 900 (calculated by 30 * 30).
